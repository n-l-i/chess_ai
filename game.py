...
def choose_move(scored_moves):
    ...
    # If there are only three moves or fewer, always pick the best move
    if len(scored_moves) <= 3:
        return scored_moves[0][0]
    # Keep only the best half of all moves
    scored_moves = scored_moves[:int(len(scored_moves)/2)]
    # Normalise all scores to start from zero and cube them to enhance diff
    lowest_score = scored_moves[-1][1]
    scored_moves = [(move,pow(score-lowest_score,3)) for (move,score) in scored_moves]
